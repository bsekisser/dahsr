...
DECLARE SUB FormatHtmLine (i$)
	...
	COMMON SHARED Sbse$
	COMMON SHARED Spacing$
	COMMON SHARED Src$
	COMMON SHARED Target$
	COMMON SHARED Tb%
	COMMON SHARED Tborder$
	COMMON SHARED Title$
	COMMON SHARED Today$
	COMMON SHARED Twidth$
...
SUB Bufferize
	...
	IF Flt% = 0 THEN
		...
		LINE INPUT #3, h$
		FormatTxtLine h$
		PRINT #2, LEFT$(h$ + " " + STRING$(MXL, 45), MXL)
		WHILE NOT EOF(3)
		LINE INPUT #3, i$
		FormatTxtLine i$
...
SUB Header (n%)
	...
	PRINT #n%, "<title>Html Quick List</title>"
	...
	PRINT #n%, "HR"
	PRINT #n%, TAB(Tb%); "{"
	PRINT #n%, TAB(Tb%); "color:#f0f;"
	PRINT #n%, TAB(Tb%); "type:dotted;"
	PRINT #n%, TAB(Tb%); "width:25%;"
	PRINT #n%, TAB(Tb%); "}"
	PRINT #n%, "LI"
	PRINT #n%, TAB(Tb%); "{"
	...
	PRINT #n%, "#shell"
	PRINT #n%, TAB(Tb%); "{"
	PRINT #n%, TAB(Tb%); "border:1px solid #00f;"
	PRINT #n%, TAB(Tb%); "margin:auto;"
	PRINT #n%, TAB(Tb%); "nowrap;"
	PRINT #n%, TAB(Tb%); "}"
	PRINT #n%, ".box"
	PRINT #n%, TAB(Tb%); "{"
	PRINT #n%, TAB(Tb%); "border:1px solid #00f;"
	...
	PRINT #n%, ".nobox"
	PRINT #n%, TAB(Tb%); "{"
	PRINT #n%, TAB(Tb%); "border:0px dashed #000;"
...
SUB Htmlize
	...
	PRINT #2, TAB(Tb% * 2); "<div class="; q; "mhdr"; q; ">Html Quick Tagname List</div>"
	...
	IF Flt% = 0 THEN
		...
		CLOSE #3: OPEN Dbse$ + f$ FOR INPUT AS #3
		IF Flt% = 0 THEN
			PRINT #2, TAB(Tb% * 2); "<div class="; q; "box"; q; ">"
			LINE INPUT #3, i$
			Decant i$, j$
			FormatHtmLine i$
			...
			ELSE
				FormatHtmLine j$
				PRINT #2, ": <span class="; q; "stxt"; q; ">"; j$; "</span>;"
			END IF
			IF NOT EOF(3) THEN
				PRINT #2, "<br>"
				PRINT #2, TAB(Tb% * 4); "<div class="; q; "nobox"; q; ">"
				WHILE NOT EOF(3)
				LINE INPUT #3, i$
				Decant i$, j$
				IF i$ = "" THEN
				...
				ELSE
					FormatHtmLine i$
					PRINT #2, TAB(Tb% * 3); "<span class="; q; "indt"; q; ">"; LCASE$(i$); "</span>";
					IF j$ = "" THEN
						j$ = ""
						PRINT #2, "<br>"
					ELSE
						...
						IF NOT EOF(3) THEN PRINT #2, "<br>" ELSE PRINT #2, ""
					END IF
				END IF
				WEND
...
SUB Initialize
	...
	IF Flt% = 0 THEN
		...
		IF x% < LEN(i$) THEN
			SELECT CASE UCASE$(LEFT$(i$, x%))
				CASE "TABLEBORDER=": Tborder$ = MID$(i$, x% + 1): EXIT FOR
				CASE "MARGINLEFT=": MarLeft$ = MID$(i$, x% + 1): EXIT FOR
				CASE "MYFILES=": MyFile$ = MID$(i$, x% + 1): EXIT FOR
				CASE "PADDING=": Padding$ = MID$(i$, x% + 1): EXIT FOR
				CASE "SPACING=": Spacing$ = MID$(i$, x% + 1): EXIT FOR
				CASE "TABLEWIDTH=": Twidth$ = MID$(i$, x% + 1): EXIT FOR
	...
	ELSE
		...
		PRINT #2, "TABLEBORDER="; Tborder$
		PRINT #2, "MARGINLEFT="; MarLeft$
		PRINT #2, "MYFILES="; MyFile$
		PRINT #2, "PADDING="; Padding$
		PRINT #2, "SPACING="; Spacing$
		PRINT #2, "TABLEWIDTH="; Twidth$
...
SUB Main
	...
	SELECT CASE UCASE$(k$)
		...
		CASE CHR$(0) + "Q": Search s%, e%, k$: EXIT DO
...
SUB Mask
	...
	ELSE
		...
		SayIt "ESC to End", Echo% + 2, Fgc%, Bgc%
	END IF
END SUB
...
SUB Scroll (s%, e%, p%)
	...
	NEXT
	IF Filename$ = "" THEN
		SayIt "TAB to Edit " + q + LCASE$(PRGM) + ".ini" + q, Echo%, Fgc%, Bgc%
	ELSE
		SayIt "TAB to Edit " + q + LCASE$(Filename$) + q, Echo%, 11, Bgc%
	END IF
END SUB
