...
SUB Calculate (bi$, de$, v%)
    IF bi$ = de$ THEN v% = 0: EXIT SUB
...
SUB Graph
    ...
    e% = 2
    FOR x% = 1 TO MXF
    v = INT((d% / VAL(Fld(0, x%)) - INT(d% / VAL(Fld(0, x%)))) * 100) / 100
    l% = LSD + (w% / MXF) * x% - (w% / MXF) + (e% / 2)
    r% = l% + (w% / MXF) - e%
    m% = Mid% * v
    IF m% <= Mid% / 2 THEN
        LINE (l%, Mid%)-(r%, Mid% - (m% / 2)), 16 - x%, BF
    ELSE
        LINE (l%, Mid%)-(r%, Mid% + (m% / 2)), 16 - x%, BF
    END IF
    NEXT
...
SUB Mask
    ...
    SayIt "ù Scroll(" + CHR$(27) + CHR$(24) + CHR$(25) + CHR$(26) + ") to Select a Name ù", ECHO + 2, Fgc%
    SayIt "ù Press ENTER to Create a Graph ù ESC to End ù", ECHO + 3, Fgc%
END SUB
...
SUB Wipe
    ...
    LINE (LSD - 15, Mid%)-(RSD + 15, Mid%), 15, B
END SUB
